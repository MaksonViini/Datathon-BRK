...
st.markdown("<h1 style='text-align: center; color: blue;'>Dados completos BRK</h1>", unsafe_allow_html=True)
st.markdown("<h4 style='text-align: center; color: blue;'>Dados completos BRK</h1>", unsafe_allow_html=True)
...
data = get_data(path)
data['Investimentos_per_capita_em_saneamento'] = data['Investimentos_per_capita_em_saneamento'].fillna(0)
filtered = get_data(path_new)
filtered['Investimentos_per_capita_em_saneamento'] = filtered['Investimentos_per_capita_em_saneamento'].fillna(0)

year = st.sidebar.slider('Ano', 2010, 2019, 2015)
# year_filtered = st.sidebar.slider('Ano', 2010, 2019, 2015)

# Filtrando os anos
data[data['Ano'] == year]
filtered[filtered['Ano'] == year]
